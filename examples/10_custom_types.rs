...
impl Person {
  pub fn new(id: Shareable<PersonId>, first_name: LangString, last_name: LangString, age: usize) -> Self {
...
trait PersonRepository {
  fn create_person(&mut self, person: Shareable<Person>) -> Result<lang_std::Void, PersonRepositoryError>;
  fn find_all_people(&self) -> Result<Vec<Shareable<Person>>, PersonRepositoryError>;
  fn find_person_by_id(&self, id: Shareable<PersonId>) -> Result<Option<Shareable<Person>>, PersonRepositoryError>;
  fn update_person_by_id(&mut self, id: Shareable<PersonId>, person: Shareable<Person>) -> Result<lang_std::Void, PersonRepositoryError>;
  fn delete_person_by_id(&mut self, id: Shareable<PersonId>) -> Result<lang_std::Void, PersonRepositoryError>;
}
...
impl PersonRepository for LocalPersonRepository {
  ...
  fn find_all_people(&self) -> Result<Vec<Shareable<Person>>, PersonRepositoryError> {
    return Ok(
      self.map.values().into_iter().map(|v| v.share()).collect::<Vec<Shareable<Person>>>()
    );
  }
...
trait PersonService {
  fn add_person(&mut self, person: Shareable<Person>) -> Result<Shareable<Person>, PersonServiceError>;
  fn find_or_add_person(&mut self, person: Shareable<Person>) -> Result<FindOrAdd<Shareable<Person>>, PersonServiceError>;
  fn find_all_people(&self) -> Result<Vec<Shareable<Person>>, PersonServiceError>;
  fn find_person_by_id(&self, id: Shareable<PersonId>) -> Result<Option<Shareable<Person>>, PersonServiceError>;
  fn update_person_by_id(&mut self, id: Shareable<PersonId>, person: Shareable<Person>) -> Result<Shareable<Person>, PersonServiceError>;
  fn delete_person_by_id(&mut self, id: Shareable<PersonId>) -> Result<lang_std::Void, PersonServiceError>;
}
...
impl StoredPersonService {
  ...
  fn _get_person_by_id(&self, id: Shareable<PersonId>) -> Result<Shareable<Person>, PersonServiceError> {
...
fn main() -> Result<lang_std::Void, PersonServiceError> {
  ...
  Console::write_line(LangString::from_owned(format!("Added person id {}", added1.borrow().id.borrow().value)));

  let found_or_added1 = person_service.find_or_add_person(person2.share())?;
  match found_or_added1 {
    FindOrAdd::Found(found) => Console::write_line(LangString::from_owned(format!("Found person id {}", found.borrow().id.borrow().value))),
    FindOrAdd::Added(added) => Console::write_line(LangString::from_owned(format!("Added person id {}", added.borrow().id.borrow().value))),
  };
  ...
  let updated1 = person_service.update_person_by_id(person1.borrow().id.share(), person1.share())?;
  Console::write_line(LangString::from_owned(format!("Updated person id {}", updated1.borrow().id.borrow().value)));

  person_service.delete_person_by_id(person2.borrow().id.share())?;
  Console::write_line(LangString::from_owned(format!("Deleted person id {}", person2.borrow().id.borrow().value)));
